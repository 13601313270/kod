 ...
 */
//echo SMARTY_SYSPLUGINS_DIR.'smarty_internal_data.php';exit;
...
class Smarty extends Smarty_Internal_TemplateBase
{
     ...
     */
    const CACHING_OFF = 0;//关闭Smarty缓存功能(默认)
    const CACHING_LIFETIME_CURRENT = 1;//用$cache_lifetime来判断缓存文件是否过期。
    const CACHING_LIFETIME_SAVED = 2;//根据缓存文件的生成时间与$cache_lifetime判断缓存文件是否过期，这个设定可以为特定的缓存文件设定特定的缓存时间。
    /**
    ...
    /**
     * 变量接受两个值：NONE和URL，当设置为NONE时，不会启动调试程序；当设定值为URL时，如果在url地址栏中发现SMARTY_DEBUG请求信息，便会打开控制台。
    ...
    /**
     * Path of debug template.如果要开启，则在网址中让这个值等于on。例如SMARTY_DEBUG=on
     * @var string
     */
    ...
    public function __construct()
    {
		...
		if (!defined('KOD_SMARTY_COMPILR_DIR') || KOD_SMARTY_COMPILR_DIR=="") {
			$this->setCompileDir('.' . DS . 'templates_c' . DS);
		}else{
			$this->setCompileDir(KOD_SMARTY_COMPILR_DIR);
		}
		if(!defined('KOD_SMARTY_TEMPLATE_DIR') || KOD_SMARTY_TEMPLATE_DIR==""){
			$this->setTemplateDir('.' . DS . 'templates' . DS);
		}else{
			$this->setTemplateDir(KOD_SMARTY_TEMPLATE_DIR);
		}
		//
        // set default dirs
    ...
    /**
     * 清空特定模板的缓存Empty cache for a specific template
     *
    ...
    public function createTemplate($template, $cache_id = null, $compile_id = null, $parent = null, $do_clone = true)
    {
		...
		//默认cache_id和compile_id Smarty对象
