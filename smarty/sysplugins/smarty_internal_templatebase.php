...
{
    /**
     * 获取一个渲染smarty的模板fetches a rendered Smarty template
     *
     * @param  string $template         模板文件，或者这个类的实例		the resource handle of the template file or template object
     * @param  mixed  $cache_id         cache id to be used with this template
     * @param  mixed  $compile_id       compile id to be used with this template
     * @param  object $parent           next higher level of Smarty variables
     * @param  bool   $display          如果为true生成页面，false返回生成的html的字符串
     * @param  bool   $merge_tpl_vars   如果为真，父模板的变量合并到当前作用域中。if true parent template variables merged in to local scope
     * @param  bool   $no_output_filter if true do not run output filter
     * @return string rendered template output
     */
	/*
	 * Exception的code和文字对应
	 * 1：您并未给Smarty的实例配置预编译文件存储路径属性compile_dir，请进行配置。您也可以在加载文件中配置静态变量【KOD_SMARTY_COMPILR_DIR】作为smarty的默认值
	 * 2：模板文件不存在
	 * */
    public function fetch($template = null, $cache_id = null, $compile_id = null, $parent = null, $display = false, $merge_tpl_vars = true, $no_output_filter = false)
    {
        ...
        // 如果传入的是一个这个类的实例，则不去创建
        $_template = ($template instanceof $this->template_class)
        ? $template
        : $this->smarty->createTemplate($template, $cache_id, $compile_id, $parent, false);
        //如果被Smarty对象确保我们使用当前缓存状态 if called by Smarty object make sure we use current caching status
        if ($this instanceof Smarty) {
            $_template->caching = $this->caching;
        }
        //合并所有变量范围为模板 merge all variable scopes into template
        if ($merge_tpl_vars) {
        ...
        //虚拟本地smarty变量 dummy local smarty variable
        ...
        if (!$this->smarty->debugging && $this->smarty->debugging_ctrl == 'URL') {//非调试模式
            if (isset($_SERVER['QUERY_STRING'])) {
                $_query_string = $_SERVER['QUERY_STRING'];
            } else {
                $_query_string = '';
            }
            if (false !== strpos($_query_string, $this->smarty->smarty_debug_id)) {//是否包含开启调试的url参数
        ...
        if (!$_template->source->exists) {
            ...
            throw new Exception("模板文件不存在 {$_template->source->type} 【{$_template->source->name}】{$parent_resource}",2);
        }
        // read from cache or render
		//Smarty::CACHING_LIFETIME_CURRENT用$cache_lifetime来判断缓存文件是否过期。
            ...
            if (!$_template->source->uncompiled) {
                ...
                if ($_template->source->recompiled) {
                    ...
                    if (!$_template->compiled->exists || ($_template->smarty->force_compile && !$_template->compiled->isCompiled)) {
						$_template->compileTemplateSource($display);
                    ...
                    if (!$_template->compiled->loaded) {
						include($_template->compiled->filepath);//加载生成的直接运行的php模板文件
						if (kod_smarty_smarty::$test || $_template->mustCompile) {//tpl模板文件被修改过，需要重新生成
							//new Smarty_Internal_Template();
							$_template->compileTemplateSource($display);//生成新的可以直接运行的php模板文件，并写入硬盘
							$code = file_get_contents($_template->compiled->filepath);
                            eval("?>" . $code);
                            unset($code);
                            $_template->compiled->isCompiled = true;
        ...
        if ($display) {//渲染页面
            if ($this->caching && $this->cache_modified_check) {//如果设置了缓存，设置http的头部何其他的一些全局变量
                ...
                //如果已经存在缓存文件，返回304
					...
					//如果还没有存在缓存文件，设置Last-Modified等设置
                    switch (PHP_SAPI) {
                        case 'cli':
                        if (/* ^phpunit */!empty($_SERVER['SMARTY_PHPUNIT_DISABLE_HEADERS'])/* phpunit$ */) {
